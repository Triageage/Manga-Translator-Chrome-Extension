...
(() => {
    ...
    switch (provider) {
      case "gemini":
        return {
          model: "gemini-3.8-flash",
          endpoint: "",
          hint: "Recommended: Google Gemini 3.8 Flash (Free tier, multimodal vision in Google AI Studio).",
  ...
  async function load() {
    ...
    // Auto-migrate from deprecated gemini-2.5-flash if previously saved
    if (state.settings.model === "gemini-2.5-flash" || !state.settings.model) {
      state.settings.model = "gemini-3.8-flash";
    }
  ...
  async function testApiConnection() {
    ...
    try {
      ...
      if (response.model) {
        ...
        const modelSelect = $("modelSelect");
        if (modelSelect) {
          const match = Array.from(modelSelect.options).find(opt => opt.value === response.model);
          modelSelect.value = match ? response.model : "custom";
        }
      }
